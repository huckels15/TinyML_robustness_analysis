...
def load_configs():
    ...
    parser.add_argument("--jsma_gamma", type=float, default=1.0, help="")
    parser.add_argument("--jsma_theta", type=str, default=[0.08], help="")
    ...
    run_cfgs = vars(cfgs)
    jsma_theta = run_cfgs['jsma_theta'].split(",")
    run_cfgs['jsma_theta'] = np.array(jsma_theta, dtype=float)
...
def main():
    ...
    y_target = random_targets(y_test, cfgs['num_classes'])

    for i, theta in enumerate(cfgs['jsma_theta']):
        print("THETA: " + str(theta))

        attack = SaliencyMapMethod(classifier=art_classifier, theta=theta, gamma=cfgs['jsma_gamma'], batch_size=1)
        x_adv_float = attack.generate(x=x_test_float, y=y_target)
