...
def load_configs():
    ...
    parser.add_argument("--autoattack_eps", type=float, default=0.004, help="")
    parser.add_argument("--autoattack_attacks_to_run", type=str, default="apgd-ce,apgd-t,fab-t,square", help="")
    ...
    run_cfgs = vars(cfgs)
    autoattack_attacks_to_run = run_cfgs['autoattack_attacks_to_run'].split(",")
    run_cfgs['autoattack_attacks_to_run'] = autoattack_attacks_to_run
...
def main():
    ...
    ann_f32_adapted = b.clone_ann_without_softmax(ann_f32)
    ann_f32_adapted = utils_tf2.ModelAdapter(ann_f32_adapted, num_classes=cfgs['num_classes'])
    ann_f32_adapted.data_format = "channels_first"
    ...
    torch_x_test_float = torch.from_numpy(x_test_float).float()
    torch_y_test = torch.from_numpy(np.where(y_test==1)[1])
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    adversary = AutoAttack(ann_f32_adapted, norm='Linf', eps=cfgs['autoattack_eps'], version='standard', is_tf_model=True, device="device")
    adversary.apgd_targeted.n_target_classes = cfgs['num_classes'] - 1
    adversary.fab.n_target_classes = cfgs['num_classes'] - 1
    adversary.attacks_to_run = cfgs['autoattack_attacks_to_run']
    #adversary.attacks_to_run = ['apgd-ce', 'apgd-t']
    x_adv_float = adversary.run_standard_evaluation(torch_x_test_float, torch_y_test, bs=1)
    x_adv_float = x_adv_float.numpy()
