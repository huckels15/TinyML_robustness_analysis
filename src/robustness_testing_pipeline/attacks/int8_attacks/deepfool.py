...
def load_configs():
    ...
    parser.add_argument("--deepfool_max_iter", type=int, default=100, help="")
    parser.add_argument("--deepfool_epsilon", type=str, default=0.008, help="")
    ...
    run_cfgs = vars(cfgs)
    deepfool_epsilon = run_cfgs['deepfool_epsilon'].split(",")
    run_cfgs['deepfool_epsilon'] = np.array(deepfool_epsilon, dtype=float)
...
def main():
    ...
    for i, max_l2_overshoot in enumerate(cfgs['deepfool_epsilon']):
        print("MAXIMUM L2 OVERSHOOT: " + str(max_l2_overshoot))

        attack = DeepFool(classifier=art_classifier, max_iter=cfgs['deepfool_max_iter'], batch_size=1,
                                nb_grads=cfgs['num_classes'], epsilon=max_l2_overshoot)
