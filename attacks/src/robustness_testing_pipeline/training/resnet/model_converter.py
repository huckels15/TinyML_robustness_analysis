...
def convert_model(model_name):
    tfmodel_path = 'src/robustness_testing_pipeline/models/' + model_name
    tfmodel = tf.keras.models.load_model(tfmodel_path)
    model_name = model_name[:-3]
    converter = tf.lite.TFLiteConverter.from_keras_model(tfmodel)
    tflite_model = converter.convert()
    open('src/robustness_testing_pipeline/models/' + model_name + '.tflite', 'wb').write(tflite_model)
    ...
    open('src/robustness_testing_pipeline/models/' + model_name + '_quant.tflite', 'wb').write(tflite_quant_model)
